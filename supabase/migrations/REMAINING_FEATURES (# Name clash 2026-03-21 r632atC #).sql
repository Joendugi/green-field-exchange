...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'unique_post_like') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'unique_post_repost') THEN
        ALTER TABLE post_reposts ADD CONSTRAINT unique_post_repost UNIQUE (user_id, post_id);
    END IF;
END $$;
