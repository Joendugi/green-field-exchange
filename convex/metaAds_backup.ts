...
export const getMetaAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    const pixelEvents = await ctx.db
      ...
      .withIndex("by_timestamp", (q) => 
        q.and(
          ...
          q.lte("timestamp", dateRangeToUse.end)
        )
      )
      .collect();
    ...
    const campaigns = await ctx.db
      ...
      .withIndex("by_created_at", (q) => 
        q.and(
          ...
          q.lte("createdAt", dateRangeToUse.end)
        )
      )
      .collect();
    ...
    const analytics = {
      ...
      averageCTR: campaigns.reduce((sum, camp) => sum + (camp.metrics?.ctr || 0), 0) / campaigns.length || 0,
      averageCPC: campaigns.reduce((sum, camp) => sum + (camp.metrics?.cpc || 0), 0) / campaigns.length || 0,
    };
